      ...
      <table className="min-w-full bg-white">
        <thead>
          <tr>
            ...
            {skillsData.map((row) => (
              <th key={row.skill} className="py-2 px-4 border">{row.skill}</th>
            ))}
          ...
          {skillsData.map((row, rowIndex) => (
            <tr key={row.skill}>
              ...
              {row.values.map((value, colIndex) => (
                <td key={colIndex} className={`py-2 px-4 border ${rowIndex === colIndex ? 'bg-blue-200' : ''}`}>
                  {value}
                </td>
              ))}
        ...
        <tfoot>
          <tr>
            ...
            <td colSpan={skillsData.length + 1} className="py-2 px-4 border font-bold">{skillsData.reduce((acc, row) => acc + row.total, 0).toFixed(2)}</td>
